...
function ContributionCalendar({ cells }: ContributionCalendarProps) {
  ...
  return (
    <DataLabSection title="Life Activity Calendar" subtitle="Each cell = 1 day. Intensity = active systems (0–7).">
          ...
          <div key={weekIdx} className="flex flex-col gap-[3px]">
            {week.map((cell, dayIdx) => (
              <div
                ...
                title={cell.date ? `${formatDateShort(cell.date)}: ${cell.activeSystemCount}/7 systems` : ''}
              />
            ))}
          </div>
        ))}
      ...
      {hoveredCell ? (
        <div className="mt-3 border-t border-border pt-3 text-xs font-mono text-slate-400">
          ...
          <span className="ml-2">{hoveredCell.activeSystemCount}/7 systems</span>
          <div className="mt-1 flex gap-3 text-[10px] flex-wrap">
            ...
            {hoveredCell.systems.learning ? <span className="text-emerald-400">Learning <Check className="inline h-3 w-3" /></span> : <span className="text-slate-600">Learning</span>}
          </div>
        </div>
      ) : null}
    </DataLabSection>
